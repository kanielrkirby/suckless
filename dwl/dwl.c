...
run(char *startup_cmd)
{
	...
	/* Autostart programs from config.h */
...
{
	char *startup_cmd_arg = NULL;
	int c;

	while ((c = getopt(argc, argv, "s:hdv")) != -1) {
		if (c == 's')
			startup_cmd_arg = optarg;
		else if (c == 'd')
			log_level = WLR_DEBUG;
		else if (c == 'v')
			die("dwl " VERSION);
	...
	setup();
	/* Use config startup_cmd if no -s arg provided */
	if (!startup_cmd_arg)
		startup_cmd_arg = (char *)default_startup_cmd;
	run(startup_cmd_arg);
	cleanup();
	return EXIT_SUCCESS;
